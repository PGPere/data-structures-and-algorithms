...
const screenForNames = (arr) => {
  let trueresults = [];
  arr.map(string => {
    let regex = /^([M][r][.]|[M][r][s][.]|[M][s][.]|[D][r][.])\s[+A-Za-z]+$/gm;
    regex.test(string) ? trueresults.push(string) : false
  });
  return trueresults;
};
...
const toTitleCase = (arr) => {
  ...
  arr.map(string => {
    trueresults.push(capitalizeFirstLetter(string));
  });
  return trueresults;
};
...
let biggerThanLuke = (arr) => {
  ...
  Object.values(arr).forEach(value => {
    if ('Luke Skywalker' !== value.name) {
      return value.mass > 77 ? temp=temp+value.name+ " - " : temp;
    }
  });
  return temp.slice(0, temp.length - 3);
};
...
const sortBy = (property, arr) => {
  if (property === 'name') {
    arr.sort( (b,a) => a.name.toUpperCase() < b.name.toUpperCase() ? 1 : -1);
    return arr;
  } else {
...
const detectTicTacToeWin = (board) => {
  let x = ((board[0][0]===board[1][0] && board[1][0]===board[2][0] && board[2][0]!=='') || (board[0][0]===board[0][1] && board[0][1]===board[0][2] && board[0][2]!=='') || (board[1][0]===board[1][1] && board[1][1]===board[1][2] && board[1][2]!=='') || (board[2][0]===board[2][1] && board[2][1]===board[2][2]) && board[2][2]!=='' || (board[0][1]===board[1][1] && board[1][1]===board[2][1] && board[2][1]!=='') || (board[0][2]===board[1][2] && board[1][2]===board[2][2] && board[2][2]!=='')|| (board[0][2]===board[1][1] && board[1][1]===board[2][0] && board[2][0]!=='')|| (board[0][0]===board[1][1] && board[1][1]===board[2][2] && board[2][2]!==''));
  return x
};
