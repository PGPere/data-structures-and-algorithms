...
const totalSum = (input) => {
  return input.reduce((accumulator,currentValue) => accumulator + currentValue.reduce((innerAccumulator, innerCurrentValue) => innerAccumulator + innerCurrentValue,0),0);
};
...
let findMaleAndFemale = (data) => {
  let result = '';
  const chrname = data.reduce( (acc, val, idx) => {
    if (val.gender === 'male' || val.gender === 'female') {
      idx < 1 ? result = val.name : result= result +' and ' + val.name;
    }
    return result;
  }, []);
  return chrname;
...
let findShortest = (data) => {
  ...
  result = result.sort((a,b) => a.height - b.height);
