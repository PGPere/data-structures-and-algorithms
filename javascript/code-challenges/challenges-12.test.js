...
const validateEmail = (email) => {
  let regex = /(^\w+\@\w+(.net|.com|.org)$)|(^\w+\.\w+\@\w+(.net|.com|.org)$)/s;
...
const validatePhoneNumber = (phoneNumber) => {
  let regex =/(^[1-9]\d{2}-\d{3}-\d{4}$)|(^\(\d{3}\)\s\d{3}-\d{4}$)|(^[1-9]\d{2}\s\d{3}\s\d{4}$)|(^[1-9]\d{2}\.\d{3}\.\d{4}$)|(^[1-9]\d{2}\s\d{3}-\d{4}$)|(^[1-9]\d{2}\s\d{3,9}$)|(^[1-9]\d{1,9}$)/s;
