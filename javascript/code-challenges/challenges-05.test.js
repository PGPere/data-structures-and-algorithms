...
const addValues = (arr) => {
  const addtogether = arr.reduce( (accumulator, val, idx) => {
...
const countNumberOfElements = (arr) => {
  const numchr = arr.reduce( (accumulator, val, idx) => {
    return idx+1;
  }, 0);
  return numchr;
};
...
const returnNames = (arr) => {
  const chrname = arr.reduce( (acc, val, idx) => {
    acc.push( val.name);
    return acc;
  }, []);
  return chrname;
