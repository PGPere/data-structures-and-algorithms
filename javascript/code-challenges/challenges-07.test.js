...
const sortStarWarsCharacters = (starWarsArr) => {
  starWarsArr = starWarsArr.sort(function (b, a) {
...
const listFoods = (recipe) => {
  ...
  for (let i = 0; i < recipe.ingredients.length; i++) {
    let idx = recipe.ingredients[i].indexOf(' ',3);
    let firstpass = recipe.ingredients[i].slice(idx);
    result.push(firstpass.slice(1));
  }
  return result;
};
