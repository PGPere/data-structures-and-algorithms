...
const firstLetters = (arr) => {
  let result =[];
  arr.map( string => {
    let temp = string.split("").splice(0,1);
    result.push(temp[0]);
  });
  return result;
};
...
const findHappiness = (arr) => {
  let results = [];
    arr.map(string => {
    let regex = /[:][)]/gs
    regex.test(string) ? results.push(string) : string
  });
  return results;
};
...
const allHappy = (arr) => {
  ...
  let temp = '';
    arr.map(string => {
    let regex = /[:][)]/gs
    regex.test(string) ? trueresults.push(true) : falseresults.push(false)
  });
  falseresults.length > 0 ? temp = false : temp = true 
  return temp;
};
